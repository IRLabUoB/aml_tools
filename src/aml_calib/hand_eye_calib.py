...
class HandEyeCalib(object):
    ...
    def calibrate(self):
        ...
        #
        for i in range(M):
            Hc[i, :, :] = wHc[i, :, :]#np.linalg.inv(wHc[i, :, :])  # wHc[i, :, :]

        for i in range(M):
            for j in range(i + 1, M):
                Hgij = np.matmul(np.linalg.inv(Hg[j, :, :]),
        ...
        for i in range(0, M):
            for j in range(i + 1, M):
                ...
                B[3 * k:3 * k + 3] = np.matmul(Rcg[:3, :3], Hcij[:3, 3].reshape(3, 1)) - Hgij[:3, 3].reshape(3,1)  # right-hand side
